...
def _build_parser() -> argparse.ArgumentParser:
    ...
    poll = sub.add_parser("telegram-poll", help="Receive /start /inbox /order on @Npeppers_bot")
...
def _telegram_poll(*, once: bool) -> int:
    ...
    if os.environ.get("TELEGRAM_MODE", "mock") != "live" or not token:
        sys.stderr.write("error: set TELEGRAM_MODE=live and TELEGRAM_BOT_TOKEN for @Npeppers_bot\n")
        return 1
    ...
    if offset_path.exists():
        try:
            ...
            offset = None
    sys.stdout.write(json.dumps({"ok": True, "polling": True, "bot": "@Npeppers_bot", "offset": offset}) + "\n")
    while True:
