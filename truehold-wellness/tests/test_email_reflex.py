...
from wellness_agent.operator_store import load_operator_chats, remember_operator
...
def test_start_links_operator_for_order_alerts():
    tg = _FakeTelegram()
    handle_telegram_update({"message": {"text": "/start", "chat": {"id": 501}}}, tg)
    assert "501" in load_operator_chats()
    assert any("order and email-reflex alerts" in item["text"] for item in tg.sent)


def test_order_marks_orders_new_and_keeps_other_categories():
    tg = _FakeTelegram()
    result = handle_telegram_update(
        {"message": {"text": "/order 2x klow", "chat": {"id": 502}}},
        tg,
    )
    ...
    assert any("TrueHold Wellness alert — order" in item.get("text", "") for item in tg.sent)
