...
from truehold.wellness_agent.compose import format_alert
from truehold.wellness_agent.models import Alert
...
def send_alert(
    ...
    *,
    ...
    Uses WELLNESS_ALERT_WEBHOOK_URL only. Does not read ALERT_WEBHOOK_URL
    (that destination belongs to the crypto agent).
    """
    ...
    if not destination:
        raise NotifyError(
            f"No {WEBHOOK_ENV} configured. Pass webhook_url or set the environment variable."
        )
